...
export function Footer() {
	...
	const { data: redditStats } = useQuery({
		queryFn: () =>
			fetch("https://www.reddit.com/r/VintageStory/comments/1ny3tvz.json").then(
				(res) =>
					res.json() as Promise<
						{
							data: {
								children: { data: { ups: number; upvote_ratio: number } }[];
							};
						}[]
					>,
			),
		...
		<footer className="border-t border-border bg-card/30 text-foreground">
			<div className="container mx-auto px-4 py-12">
				<div className="grid grid-cols-1 md:grid-cols-3 gap-8">
					<div className="space-y-4">
						...
						<p className="text-muted-foreground text-sm">
							The ultimate launcher for Vintage Story, built by the community
							for the community.
						</p>
						<p className="text-muted-foreground text-sm">
							Story Forge is not affiliated with or endorsed by{" "}
							<a
						...
						<ul className="space-y-2 text-sm text-muted-foreground">
							<li>
								<a
									...
									href={
										assets.find((asset) => asset.platform === "windows-x86_64")
											?.url
									}
								>
									Windows
							...
							<li>
								<a
									className="hover:text-foreground transition-colors"
									href={
										assets.find((asset) => asset.platform === "darwin-aarch64")
											?.url
									}
								>
									macOS (Apple Silicon)
							...
							<li>
								<a
									className="hover:text-foreground transition-colors"
									href={
										assets.find((asset) => asset.platform === "darwin-x86_64")
											?.url
									}
								>
									macOS (Intel)
							...
							<li>
								<a
									className="hover:text-foreground transition-colors"
									href={
										assets.find((asset) => asset.platform === "linux-x86_64")
											?.url
									}
								>
									Linux (x86_64)
							...
							<li>
								<a
									className="hover:text-foreground transition-colors"
									href={
										assets.find((asset) => asset.platform === "linux-aarch64")
											?.url
									}
								>
									Linux (ARM64)
						...
						<div className="flex space-y-4 flex-col">
							...
							>
								...
								<span className="text-sm">
									{redditStats?.[0]?.data?.children[0]?.data?.ups ?? 0} Upvotes
								</span>
							</a>
							<a
								className="text-muted-foreground hover:text-foreground transition-colors"
				...
				<div className="border-t border-border mt-8 pt-8 text-center text-sm text-muted-foreground">
					<p>
						&copy; 2025 Story Forge. Open source software released under the
						GPLv3 License.
					</p>
				</div>
			</div>
