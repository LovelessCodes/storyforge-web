import { Monitor, Apple, Smartphone, DownloadIcon } from "lucide-react"
...
const platformIcon = (platform: keyof typeof platformGroups, props?: React.SVGProps<SVGSVGElement>) => {
    if (platform === "Windows") return (
        <svg fill="currentColor" height="14px" width="14px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 498.57 498.57" {...props}>
    ...
    if (platform === "MacOS") return (
        <svg fill="currentColor" width="14px" height="14px" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg" version="1.1" {...props}>
    ...
    if (platform === "Linux") return (
        <svg fill="currentColor" width="14px" height="14px" viewBox="0 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg" {...props}>
            <title>linux</title>
...
const platformOptions: {
    ...
    "darwin-aarch64": { platform: "Apple Silicon", icon: <Apple className="w-8 h-8 text-primary" /> },
    "linux-x86_64": { platform: "AMD", icon: <Monitor className="w-8 h-8 text-primary" /> },
    "darwin-x86_64": { platform: "Intel", icon: <Apple className="w-8 h-8 text-primary" /> },
    "linux-aarch64": { platform: "ARM", icon: <Monitor className="w-8 h-8 text-primary" /> },
    "windows-x86_64": { platform: "x64", icon: <Smartphone className="w-8 h-8 text-primary" /> },
}

export function DownloadSection() {
    const { data: downloads } = useQuery({
        queryKey: ['latest-downloads'],
        queryFn: () => fetch("https://api.github.com/repos/lovelesscodes/storyforge/releases/latest").then(res => res.json() as Promise<DownloadInfo>),
    })
    ...
    return (
        <section id="downloads" className="mb-20 md:px-64">
            <div className="text-center mb-6">
                <h2 className="text-4xl font-bold mb-4 text-foreground">Download Story Forge</h2>
                <p className="text-muted-foreground text-lg">Choose your platform and start your Vintage Story adventure</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {platformGroups && Object.entries(platformGroups).map(([group, groupAssets]) => (
                    <div key={group} className="flex flex-col gap-4 items-center">
                        ...
                        {groupAssets.map(assetKey => {
                            ...
                            return (
                                <a key={assetKey} href={asset.url} className="w-full" target="_blank" rel="noopener noreferrer">
                                    <button
                                        className="group relative w-auto cursor-pointer overflow-hidden rounded-full w-full border text-foreground bg-background p-2 px-6 text-center font-semibold"
                                    >
                                        <div className="flex items-center justify-center gap-2">
                                            <div className="h-2 w-2 rounded-full bg-primary transition-all opacity-0 duration-300 group-hover:opacity-100 group-hover:scale-[100.8]"></div>
                                            <span className="inline-block transition-all duration-300 group-hover:translate-x-12 group-hover:opacity-0">
                                                {platformOptions[asset.platform]?.platform}
                                            </span>
                                        </div>
                                        <div className="absolute top-0 z-10 flex h-full w-full translate-x-12 items-center justify-center gap-2 text-primary-foreground opacity-0 transition-all duration-300 group-hover:-translate-x-5 group-hover:opacity-100">
                                            <span>{platformOptions[asset.platform]?.platform}</span>
